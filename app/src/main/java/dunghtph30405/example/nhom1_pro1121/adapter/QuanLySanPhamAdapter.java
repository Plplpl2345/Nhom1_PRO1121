...
public class QuanLySanPhamAdapter extends RecyclerView.Adapter<QuanLySanPhamAdapter.ViewHolder> {
    ...
    private ArrayList<sanpham> fullList; // Added for filtering
    private SanPhamDAO sanPhamDAO;

    public QuanLySanPhamAdapter(Context context, ArrayList<sanpham> list, SanPhamDAO sanPhamDAO) {
        this.context = context;
        this.list = list;
        this.fullList = new ArrayList<>(list); // Initialize fullList for filtering
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.txt_xemthem.setOnClickListener(v -> showdialogXemThem(product));
        ...
        holder.btn_delete.setOnClickListener(v -> {
            ...
            builder.setPositiveButton("YES", (dialog, which) -> {
                boolean check = sanPhamDAO.deleteSP(product.getMasp());
                if (check) {
                    loadData();
        ...
        holder.btn_update.setOnClickListener(v -> dialogUpdate(product));
    }
    ...
    public void loadData() {
        list.clear();
        list.addAll(sanPhamDAO.selectSANPHAM());
        fullList = new ArrayList<>(list); // Update fullList after loading
        notifyDataSetChanged();
    }
    ...
    public void dialogUpdate(sanpham sp) {
        ...
        EditText edt_hinhanh_ud = view.findViewById(R.id.edt_hinhanh_ud); // Added image field
        ...
        builder.setNegativeButton("Cập Nhật", (dialog, which) -> {
            try {
                ...
                String giaText = edt_giasp_ud.getText().toString();
                int giasp_ud = giaText.isEmpty() ? 0 : Integer.parseInt(giaText);
                ...
                if (tensp_ud.isEmpty() || giaText.isEmpty() || thuonghieu_ud.isEmpty() ||
                        xuatxu_ud.isEmpty() || kichthuocmanhinh_ud.isEmpty() || mausac_ud.isEmpty() ||
                        trongluong_ud.isEmpty() || chatlieu_ud.isEmpty() || cpu_ud.isEmpty() ||
                        ocung_ud.isEmpty() || ram_ud.isEmpty() || rom_ud.isEmpty() ||
                        card_ud.isEmpty() || tocdocpu_ud.isEmpty() || congusb_ud.isEmpty() ||
                        vantay_ud.isEmpty() || hinhanh_ud.isEmpty()) {
                    Toast.makeText(context, "Không được để trống dữ liệu", Toast.LENGTH_SHORT).show();
                } else {
                    boolean check = sanPhamDAO.updateSP(masp, tensp_ud, giasp_ud, thuonghieu_ud,
                ...
                Toast.makeText(context, "Giá phải là số hợp lệ", Toast.LENGTH_SHORT).show();
            }
        });
